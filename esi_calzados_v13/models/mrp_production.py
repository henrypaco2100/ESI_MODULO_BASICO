...
class MrpProduction(models.Model):
    ...
    esi_production_group_id = fields.Many2one(
        'esi.production.group', string='Grupo de producción ESI', copy=False, index=True, ondelete='set null'
    )
    esi_production_group_line_id = fields.Many2one(
        'esi.production.group.line', string='Línea del grupo ESI', copy=False, ondelete='set null'
    )
    esi_sale_order_id = fields.Many2one(
        related='esi_production_group_id.sale_order_id', string='Pedido de venta', store=True, readonly=True
    )
    esi_customer_id = fields.Many2one(
        related='esi_production_group_id.partner_id', string='Cliente', store=True, readonly=True
    )
    esi_salesperson_id = fields.Many2one(
        related='esi_production_group_id.salesperson_id', string='Vendedor', store=True, readonly=True
    )
    esi_production_manager_id = fields.Many2one(
        related='esi_production_group_id.production_manager_id', string='Encargado de producción', store=True, readonly=True
    )
    esi_destajo_ids = fields.One2many('esi.calzado.destajo', 'production_id', string='Destajos (histórico)')
    ...
    def _compute_esi_summary(self):
        for production in self:
            ...
            # Los destajos dejaron de formar parte del costo de producción.
            # Se conservan campos históricos en cero para no romper bases ya actualizadas.
            total = material_cost + (production.esi_other_cost or 0.0)
            production.esi_destajo_count = 0
            ...
            production.esi_destajo_cost = 0.0
            production.esi_destajo_confirmed_cost = 0.0
    ...
    def button_mark_done(self):
        # Los destajos son ahora gastos independientes; no bloquean el cierre ni
        # se cargan en mrp_account.extra_cost. La valoración automática de
        # materiales/producto terminado continúa validándose normalmente.
