...
class SdInheritResCompany(models.Model):
    ...
    def create_management_company(self):
        if self.env['res.users'].has_group('esi_sd_stock_valuation_layer.sd_grupo_permiso_administrar_management_company'):
            vals = {
    ...
    def action_view_management_company(self):
        ...
        if len(self.sd_management_ids.ids) != 1:
            ...
            return {
                ...
                'res_model': 'management.company',
                'view_mode': 'form',
                'views': [[self.env.ref('esi_sd_stock_valuation_layer.sd_form_management_company_order').id, 'form']],
